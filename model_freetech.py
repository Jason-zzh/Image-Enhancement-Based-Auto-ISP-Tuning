...
class CSALayer(nn.Module):
    def __init__(self, dim, num_virtual_cams=5):
        super().__init__()
        self.weight = nn.Parameter(torch.ones(num_virtual_cams, dim, 1, 1))
        self.bias = nn.Parameter(torch.zeros(num_virtual_cams, dim, 1, 1))

    def forward(self, x, cam_id=0):
        return x * self.weight[cam_id] + self.bias[cam_id]

class IMNRLayer(nn.Module):
    def __init__(self, dim): super().__init__(); self.conv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True)

    def forward(self, x): return self.conv(x)

class OMNRLayer(nn.Module):
    def __init__(self, dim): super().__init__(); self.conv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True); nn.init.zeros_(self.conv.weight); nn.init.zeros_(self.conv.bias)

    def forward(self, x): return self.conv(x)

class RepNRBlock(nn.Module):
    def __init__(self, dim, num_virtual_cams=5, target_cam=False):
        super().__init__()
        self.dim = dim
        self.csa = CSALayer(dim, num_virtual_cams) if not target_cam else nn.Identity()
        self.imnr = IMNRLayer(dim)
        self.omnr = OMNRLayer(dim)
        self.act = nn.GELU()

    def forward(self, x, cam_id=0, mode='train'):
        feat = self.csa(x, cam_id) if hasattr(self, 'csa') else x
        imnr_out = self.imnr(feat)
        omnr_out = self.omnr(x)
        out = self.act(imnr_out + omnr_out)
        return out

    def reparameterize(self):
        fused_conv = nn.Conv2d(self.dim, self.dim, 3, 1, 1)
        if hasattr(self, 'csa') and hasattr(self.csa, 'weight'):
            # 简化 diag(W_csa) * W_imnr + W_omnr (需扩展到 full kernel)
            w_csa = self.csa.weight[0].view(self.dim, 1, 1, 1)  # diag approx
            fused_conv.weight.data = w_csa * self.imnr.conv.weight + self.omnr.conv.weight
            fused_conv.bias.data = self.imnr.conv.bias + self.omnr.conv.bias
        else:
            fused_conv = self.imnr.conv  # fallback
        return fused_conv
...
class Conv_Transformer(nn.Module):
    ...
    """
    def __init__(self, in_channel,num_heads=8,ffn_expansion_factor=2,use_rep_nr=True,num_virtual_cams=5):
        super().__init__()
        ...
        self.use_rep_nr = use_rep_nr
        if use_rep_nr:
            self.rep_nr = RepNRBlock(in_channel, num_virtual_cams)
    ...
    def forward(self, x, cam_id=0):
        ...
        # 插入 RepNR
        if self.use_rep_nr:
            conv = self.rep_nr(conv, cam_id)  # 主要到 conv 分支
            trans = self.rep_nr(trans, cam_id)  # 如果想全局噪声移除
...
class RawFormer(nn.Module):
    ...
    def __init__(self,inp_channels=1,out_channels=3,dim=48,num_heads=[8,8,8,8],ffn_expansion_factor=2,use_rep_nr=True,num_virtual_cams=5,pretrain_mode=False):
        super(RawFormer, self).__init__()
        # self.pixelunshuffle = nn.PixelUnshuffle(2)
        self.pretrain_mode = pretrain_mode
        self.num_virtual_cams = num_virtual_cams if use_rep_nr else 0
        self.lrelu = nn.LeakyReLU(0.2, inplace=False)
        self.embedding = nn.Conv2d(inp_channels*4,dim,kernel_size=3, stride=1, padding=1)
        self.conv_tran1 = Conv_Transformer(dim,num_heads[0],ffn_expansion_factor,use_rep_nr,num_virtual_cams)
        self.down1 = Downsample(dim)
        self.conv_tran2 = Conv_Transformer(dim*2,num_heads[1],ffn_expansion_factor,use_rep_nr,num_virtual_cams)
        self.down2 = Downsample(dim*2)
        self.conv_tran3 = Conv_Transformer(dim*4,num_heads[2],ffn_expansion_factor,use_rep_nr,num_virtual_cams)
        self.down3 = Downsample(dim*4)
        self.conv_tran4 = Conv_Transformer(dim*8,num_heads[3],ffn_expansion_factor,use_rep_nr,num_virtual_cams)

        self.up1 = nn.ConvTranspose2d(dim*8, dim*4, 2, stride=2)
        self.channel_reduce1 = nn.Conv2d(dim*8,dim*4,1,1)
        self.conv_tran5 = Conv_Transformer(dim*4,num_heads[2],ffn_expansion_factor,use_rep_nr,num_virtual_cams)
        self.up2 = nn.ConvTranspose2d(dim*4, dim*2, 2, stride=2)
        self.channel_reduce2 = nn.Conv2d(dim * 4, dim * 2, 1, 1)
        self.conv_tran6 = Conv_Transformer(dim*2,num_heads[1],ffn_expansion_factor,use_rep_nr,num_virtual_cams)
        self.up3 = nn.ConvTranspose2d(dim*2, dim*1, 2, stride=2)
        self.channel_reduce3 = nn.Conv2d(dim * 2, dim * 1, 1, 1)
        self.conv_tran7 = Conv_Transformer(dim,num_heads[0],ffn_expansion_factor,use_rep_nr,num_virtual_cams)
