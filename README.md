...
> 注意：本项目使用的数据集归 Freetch 所有，不随仓库提供。README 中只说明数据目录格式和运行方式，不包含数据下载链接。
...
如实际数据的分辨率、header、bit depth 或 Bayer pattern 不同，需要修改 `load_dataset_freetch.py` 和 demo 脚本中的对应参数。
...
- RawFormer baseline：端到端 RAW 图像复原/增强模型。
- LED / RepNR 思路：用于低光增强中的可重参数化噪声抑制。
